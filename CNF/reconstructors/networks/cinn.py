...
class CINN(pl.LightningModule):
    ...
    def __init__(self, in_ch:int, img_size, operator,
        ...
        self.criterion = CINNNLLLoss(distribution=self.hparams.sample_distribution)
 ...
 log_jac(log_jac_det) : log det of the Jacobian  log |det dz/dx | <==== Eq.(17&18)
        ...
        # direction (Z|Y) -> X
        if rev:
            x, log_jac_inv = self.cinn(cinn_input, c=self.cond_net(cond_input), rev=rev)
            if cut_ouput:
                return x[:,:,:self.op.domain.shape[0],:self.op.domain.shape[1]],log_jac_inv
            else:    
                return x,log_jac_inv
        # direction (X|Y) -> Z
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        y, gt = batch  # (b,1,1000,513)   (b,1,362,362)

        # pad gt image to the right size
        gt = self.img_padding(gt)   # (b,1,384,384)
    ...
    def training_epoch_end(self, result):
        ...
        self.logger.experiment.add_image("ground truth",img_grid, global_step=self.current_epoch)
        ...
        #     self.logger.experiment.add_graph(self.forward(), (z,y))
        ...
        with torch.no_grad():
            ...
            for i, c in enumerate(conds): 
                ...
                self.logger.experiment.add_image(f"cond_level_{i}",
...
def _add_conditioned_section(nodes, downsampling_level, num_blocks, cond, coupling, act_norm, permutation, clampling=2):
    """
